...
def run_all(cfg):
    ...
    n_progams = len(cfg['homework5_programs'])
    n_tests = len(cfg['deterministic_tests']) + len(cfg['hoppl_deterministic_tests']) + len(cfg['probabilistic_tests'])
    if wandb_run and (n_tests + n_progams > 0):
        wandb.init(project='HW5', entity='cs532-2022')
